*log using "C:\My Documents\Rawls\MEX\MEX_`aa'_growth.smcl", replace
*log using "C:\Documents and Settings\Pete Klenow\My Documents\My Documents\Rawls\MEX\MEX_`aa'_growth.smcl", replace
...
*****************************************************************************
*		 FIND THE GROWTH LAMBDA IN THE MEX BETWEEN 1984 - 2002				*
* This program computes the % increase in consumption that individuals 	    *
* need in 1984 to be as well of as they are in 2002 						*
*****************************************************************************

*** STEP I: Calculate base (1984) values for CbarMEX, LbarMEX, CineqMEX, 
* LineqMEX etc using 1984 data and 1990 survival rates. 
...
*** STEP II, use 2002 data with 1990 survival rates and combine with Part I 
...
*use "C:\Documents and Settings\Pete Klenow\My Documents\My Documents\Rawls\MEX\MEX_`aa'.dta", clear
...
* 	`aaaa' Mexican pesos to constant international prices, incorporate govt consumption
...
*	merge m:1 country_code year age using "Survival Rates\survivalMEX90.dta"
	merge m:1 country_code year age using "$survival_ind/survivalMEX90.dta"

	*merge m:1 country_code year age using "C:\Documents and Settings\Pete Klenow\My Documents\My Documents\Rawls\Survival Rates\survivalMEX90.dta"
	keep if year==1990
	replace year=`aaaa'
}
else {
...
scalar Cbar_MEX_`aa'=exp(log_cbar_`aa')
scalar Cineq_MEX_`aa'=Elogc_`aa'-log_cbar_`aa'
...
*generate log_lbar, the log of average la, with s_MEXa as weights
* 
...
scalar epsLbar_MEX_`aa'=eps_lbar_`aa' 
scalar Lineq_MEX_`aa'=-$theta*(Eeps_l_`aa'-eps_lbar)
...
**open 2002 consumption data file. Discard age > 100 and renormalize weights
quietly {
use "MEX_`bb'.dta", clear
*use "C:\Documents and Settings\Pete Klenow\My Documents\My Documents\Rawls\MEX\MEX_`bb'.dta", clear
...
collapse (mean) logca=logca logla=logla eps_la la=la ca=ca s_MEXa=s_MEXa  delta_s_MEXa= delta_s_MEXa, by(country_code year age)
...
scalar log_lambda_C_avg = log_cbar - ln(Cbar_MEX_`aa')

* Calculate average leisure term
scalar log_lambda_L_avg = -$theta*(eps_lbar - epsLbar_MEX_`aa')

* Calculate consumption inequality term
scalar log_lambda_C_ineq = Elogc - log_cbar - Cineq_MEX_`aa'

* Calculate leisure inequality term
scalar log_lambda_L_ineq = -$theta*(Eeps_l-eps_lbar) - Lineq_MEX_`aa'
...
* Lambda Decomposition for MEX `aaaa'-`bbbb'
