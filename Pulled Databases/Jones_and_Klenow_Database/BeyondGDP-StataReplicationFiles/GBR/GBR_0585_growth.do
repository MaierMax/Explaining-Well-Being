*log using "C:\Users\klenow\Documents\Rawls\GBR\GBR_05_growth.smcl", replace
*log using "C:\Users\klenow.ECON\Documents\Rawls\GBR\GBR_05_growth.smcl", replace


*****************************************************************************
*		 FIND THE GROWTH LAMBDA IN U.K BETWEEN 1985 - 2005				*
* This program computes the % reduction in consumption that individuals are *
* willing to undertake in 2005 and still be as well of as they were in 1985 *
*****************************************************************************

*** The program proceeds in 2 steps

*** STEP I: Calculate base (2005) values for CbarGBR, LbarGBR, CineqGBR, 
* LineqGBR etc using 2005 data and 2005 survival rates

*** STEP II, use 1985 data with 2005 survival rates and combine with Part I 
* to get growth lambda's. NOTE: Because 1985 data for consumption and 
...
quietly {
*use "C:\Users\klenow\Documents\Rawls\GBR\GBR_05.dta", clear
use "GBR_05.dta", clear
...
* 	2005 GBR to constant international prices, incorporate govt consumption
...
*merge m:1 country_code year age using "C:\Users\klenow\Documents\Rawls\Survival Rates\survivalGBR05.dta"
merge m:1 country_code year age using "$survival_ind/survivalGBR05.dta"
keep if year==2005
keep if country_code=="GBR"
drop _merge
...
collapse (mean) logca=logca logla=logla eps_la=eps_la ca=ca la=la s_GBRa=s_GBRa  delta_s_GBRa= delta_s_GBRa, by(country_code year age)
...
*generate log_cbar and log_lbar, the log of average ca and la, with s_GBRa as weights
* 

gen cbar_a = s_GBRa*ca
egen cbar = total(cbar_a)
di cbar
gen log_cbar = ln(cbar)

gen lbar_a = s_GBRa*la
...
gen logc_a = logca*s_GBRa
egen Elogc = total(logc_a)

gen logl_a = logla*s_GBRa
egen Elogl = total(logl_a)

**************************
gen epsl_a = eps_la*s_GBRa
...
scalar Cbar_GBR_05=exp(log_cbar_05) //This is 2005 micro data on consumption weighted by 2005 survival rates
scalar epsLbar_GBR_05=eps_lbar_05 //This is 2005 micro data on leisure weighted by 2005 survival rates
scalar Cineq_GBR_05=Elogc_05-log_cbar_05
scalar Lineq_GBR_05=-$theta*(Eeps_l_05-eps_lbar)
...
****** Lambda's are found in 2 steps because 1985 consumption and expenditure data are different ******
...
**open 1985 consumption data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\GBR\GBR_85.dta", clear
use "GBR_85.dta", clear
...
*Now that we have allocated expenditures to individuals, convert individual consumption from 
* 	1985 GBR to constant international prices, incorporate govt consumption
* 	using Penn World Tables 6.3, and scale to year $baseyear US $
...
**Compute stdev of ln(leisure) with GBR base period demographic weights
foreach var in lnchat {
qui {
su `var' [weight=s_GBRa*wbar]
return list
...
collapse (mean) logca=logca ca=ca s_GBRa=s_GBRa  delta_s_GBRa= delta_s_GBRa, by(country_code year age)
...
*generate log_cbar, the log of average ca, with s_GBRa as weights
...
scalar log_lambda_C_avg = log_cbar - ln(Cbar_GBR_05)


* Calculate consumption inequality term
scalar log_lambda_C_ineq = Elogc - log_cbar - Cineq_GBR_05
...
**open 1985 leisure data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\GBR\GBR_85.dta", clear
use "GBR_85.dta", clear
replace age=age+1
...
collapse (mean)logla=logla eps_la la=la s_GBRa=s_GBRa delta_s_GBRa=delta_s_GBRa, by(country_code year age)
...
scalar log_lambda_L_avg = -$theta*(eps_lbar - epsLbar_GBR_05)

* Calculate leisure inequality term
scalar log_lambda_L_ineq = -$theta*(Eeps_l-eps_lbar) - Lineq_GBR_05
...
* Lambda Decomposition for GBR 1985-2005
