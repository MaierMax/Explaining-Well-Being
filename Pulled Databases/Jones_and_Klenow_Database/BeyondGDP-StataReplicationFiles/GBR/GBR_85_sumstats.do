...
sort hhref persno
keep persno hhref year yrm age L hours
merge m:m hhref yrm using "merged_hh_resid.dta"
drop _merge
drop if year !=1985
*saveold "GBR_85.dta", replace

keep persno year hhref L age hours Y_D C C_p numads numhhkid grosfac
rename persno pcode
rename hhref hhid
rename Y_D hhinc
...
/*
hhsize_rui is smaller, which means hhsize includes people whose info is not in the survey
...
rename C_p hhexp
rename grosfac weight

* normalize weights to sum to 1


keep hhid pcode hhsize age leisure hhexp weight hhinc numhhkid
order hhid pcode hhsize age hhinc leisure hhexp weight numhhkid
rename weight weight_sample
sort hhid pcode
saveold "GBR_85_org.dta", replace

*****************************************************************
** Create the data for the young
*****************************************************************
use "GBR_85_org.dta",clear
...
saveold "GBR_85_young.dta", replace

**********************************************************************
*  Append the original data with the created young data
**********************************************************************
use "GBR_85_org.dta",clear
app using "GBR_85_young.dta"
...
format weight %11.4e
outfile using "GBR_85.txt", replace wide
