*log using "C:\Users\Siddharth\Documents\phd\pete\Rawls\stata_copy\stata\CFE Labor\Growth\IND_05_growth_log.log",replace
*log using "C:\Documents and Settings\Sid\My Documents\phd\pete\Rawls\stata_copy\stata\growth\growth_log.smcl", replace
*log using "C:\Users\klenow\Documents\Rawls\IND\IND_05_growth.smcl", replace

*****************************************************************************
*		 FIND THE GROWTH LAMBDA IN INDIA BETWEEN 1983 - 2005				*
* This program computes the % reduction in consumption that individuals are *
* willing to undertake in 2005 and still be as well of as they were in 1983 *
*****************************************************************************

*** The program proceeds in 2 steps

*** STEP I: Calculate base (2005) values for CbarIND, LbarIND, CineqIND, 
* LineqIND etc using 2005 data and 2005 survival rates

*** STEP II, use 1983 data with 2005 survival rates and combine with Part I 
* to get growth lambda's. NOTE: Because 1983 data for consumption and 
...
*merge m:1 country_code year age using "C:\Users\Siddharth\Documents\phd\pete\Rawls\stata_copy\stata\growth\survivalIND05.dta"
*merge m:1 country_code year age using "C:\Documents and Settings\Sid\My Documents\phd\pete\Rawls\stata_copy\stata\growth\survivalIND05.dta"
*merge m:1 country_code year age using "C:\Users\klenow\Documents\Rawls\Survival Rates\survivalIND05.dta"
merge m:1 country_code year age using "$survival_ind/survivalIND05.dta"
keep if year==2005
keep if country_code=="IND"
drop _merge
...
order country country_code year age logca logla ca la wbar 


*collapse dataset to have one observation(row) per age group.
 
collapse (mean) logca=logca logla=logla eps_la=eps_la ca=ca la=la s_INDa=s_INDa delta_s_INDa= delta_s_INDa, by(country_code year age)
...
*generate log_cbar and log_lbar, the log of average ca and la, with s_INDa as weights
* 
...
scalar log_lbar_05=log_lbar
scalar Elogc_05=Elogc
scalar Elogl_05=Elogl

scalar Eepsl_05=Eeps_l

scalar Cbar_IND_05 = exp(log_cbar_05) //This is 2005 micro data on consumption weighted by 2005 survival rates
scalar Cineq_IND_05 = Elogc_05-log_cbar_05

scalar Lbar_IND_05 = lbar
scalar Lineq_IND_05 = -(Eeps_l-eps_lbar)
...
****** Lambda's are found in 2 steps because 1983 consumption and expenditure data are different ******
...
**open 1983 consumption data file. Discard age > 100 and renormalize weights
quietly {
...
scalar log_lambda_C_avg = log_cbar - ln(Cbar_IND_05)


* Calculate consumption inequality term
scalar log_lambda_C_ineq = Elogc - log_cbar - Cineq_IND_05
...
**open 1983 leisure data file. Discard age > 100 and renormalize weights
quietly {
...
scalar log_lambda_L_avg = -$theta*(eps_lbar - ((1-Lbar_IND_05)^((1+$epsilon)/$epsilon))/((1+$epsilon)/$epsilon))

* Calculate leisure inequality term
scalar log_lambda_L_ineq = $theta*(-(Eeps_l-eps_lbar)-Lineq_IND_05)
