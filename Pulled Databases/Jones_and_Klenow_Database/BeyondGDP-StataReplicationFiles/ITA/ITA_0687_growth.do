*log using "C:\Users\klenow\Documents\Rawls\ITA\ITA_06_growth.smcl", replace
*log using "C:\Users\klenow.ECON\Documents\Rawls\ITA\ITA_06_growth.smcl", replace


*****************************************************************************
*		 FIND THE GROWTH LAMBDA IN Italy BETWEEN 1987 - 2006				*
* This program computes the % reduction in consumption that individuals are *
* willing to undertake in 2006 and still be as well of as they were in 1987 *
*****************************************************************************

*** The program proceeds in 2 steps

*** STEP I: Calculate base (2006) values for CbarITA, LbarITA, CineqITA, 
* LineqITA etc using 2006 data and 2006 survival rates

*** STEP II, use 1987 data with 2006 survival rates and combine with Part I 
* to get growth lambda's. NOTE: Because 1987 data for consumption and 
* leisure have different weights in IND, this 2nd step is itself done in 2 sub-steps


**********************************************************
*********				STEP I  			 *************
********* 2006 data with 2006 survival rates *************
**********************************************************
...
quietly {
*use "C:\Users\klenow\Documents\Rawls\ITA\ITA_06.dta", clear
use "ITA_06.dta", clear
...
* 	2006 ITA to constant international prices, incorporate govt consumption
...
*merge m:1 country_code year age using "C:\Users\klenow\Documents\Rawls\Survival Rates\survivalITA06.dta"
merge m:1 country_code year age using "$survival_ind/survivalITA06.dta"
keep if year==2006
keep if country_code=="ITA"
drop _merge
...
collapse (mean) logca=logca logla=logla eps_la=eps_la ca=ca la=la s_ITAa=s_ITAa  delta_s_ITAa= delta_s_ITAa, by(country_code year age)
...
*generate log_cbar and log_lbar, the log of average ca and la, with s_ITAa as weights
* 

gen cbar_a = s_ITAa*ca
egen cbar = total(cbar_a)
di cbar
gen log_cbar = ln(cbar)

gen lbar_a = s_ITAa*la
...
gen logc_a = logca*s_ITAa
egen Elogc = total(logc_a)

gen logl_a = logla*s_ITAa
egen Elogl = total(logl_a)

**************************
gen epsl_a = eps_la*s_ITAa
...
scalar log_cbar_06=log_cbar
scalar eps_lbar_06=eps_lbar
scalar Elogc_06=Elogc
scalar Eeps_l_06=Eeps_l

scalar Cbar_ITA_06=exp(log_cbar_06) //This is 2006 micro data on consumption weighted by 2006 survival rates
scalar epsLbar_ITA_06=eps_lbar_06 //This is 2006 micro data on leisure weighted by 2006 survival rates
scalar Cineq_ITA_06=Elogc_06-log_cbar_06
scalar Lineq_ITA_06=-$theta*(Eeps_l_06-eps_lbar)
...
****** Lambda's are found in 2 steps because 1987 consumption and expenditure data are different ******
...
**open 1987 consumption data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\ITA\ITA_87.dta", clear
use "ITA_87.dta", clear
...
*Now that we have allocated expenditures to individuals, convert individual consumption from 
* 	1987 ITA to constant international prices, incorporate govt consumption
* 	using Penn World Tables 6.3, and scale to year $baseyear US $
...
**Compute stdev of ln(leisure) with ITA base period demographic weights
foreach var in lnchat {
qui {
su `var' [weight=s_ITAa*wbar]
return list
...
collapse (mean) logca=logca ca=ca s_ITAa=s_ITAa  delta_s_ITAa= delta_s_ITAa, by(country_code year age)
...
*generate log_cbar, the log of average ca, with s_ITAa as weights
...
scalar log_lambda_C_avg = log_cbar - ln(Cbar_ITA_06)


* Calculate consumption inequality term
scalar log_lambda_C_ineq = Elogc - log_cbar - Cineq_ITA_06
...
**open 1987 leisure data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\ITA\ITA_87.dta", clear
use "ITA_87.dta", clear
replace age=age+1
...
collapse (mean)logla=logla eps_la la=la s_ITAa=s_ITAa delta_s_ITAa=delta_s_ITAa, by(country_code year age)
...
scalar log_lambda_L_avg = -$theta*(eps_lbar - epsLbar_ITA_06)

* Calculate leisure inequality term
scalar log_lambda_L_ineq = -$theta*(Eeps_l-eps_lbar) - Lineq_ITA_06
...
* Lambda Decomposition for ITA 1987-2006
