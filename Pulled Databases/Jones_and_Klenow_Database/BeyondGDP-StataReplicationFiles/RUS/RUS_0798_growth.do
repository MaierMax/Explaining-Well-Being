*log using "C:\Users\klenow\Documents\Rawls\RUS\RUS_07_growth.smcl", replace
*log using "C:\Users\klenow.ECON\Documents\Rawls\RUS\RUS_07_growth.smcl", replace


*****************************************************************************
*		 FIND THE GROWTH LAMBDA IN RUS BETWEEN 1998 - 2007				*
* This program computes the % reduction in consumption that individuals are *
* willing to undertake in 2007 and still be as well of as they were in 1998 *
*****************************************************************************

*** The program proceeds in 2 steps

*** STEP I: Calculate base (2007) values for CbarRUS, LbarRUS, CineqRUS, 
* LineqRUS etc using 2007 data and 2007 survival rates

*** STEP II, use 1998 data with 2007 survival rates and combine with Part I 
* to get growth lambda's. NOTE: Because 1998 data for consumption and 
* leisure have different weights in IND, this 2nd step is itself done in 2 sub-steps


**********************************************************
*********				STEP I  			 *************
********* 2007 data with 2007 survival rates *************
**********************************************************
...
quietly {
*use "C:\Users\klenow\Documents\Rawls\RUS\RUS_07.dta", clear
use "RUS_07.dta", clear
...
* 	2007 RUS to constant international prices, incorporate govt consumption
...
*merge m:1 country_code year age using "C:\Users\klenow\Documents\Rawls\Survival Rates\survivalRUS07.dta"
merge m:1 country_code year age using "$survival_ind/survivalRUS07.dta"
keep if year==2007
keep if country_code=="RUS"
drop _merge
...
collapse (mean) logca=logca logla=logla eps_la=eps_la ca=ca la=la s_RUSa=s_RUSa  delta_s_RUSa= delta_s_RUSa, by(country_code year age)
...
*generate log_cbar and log_lbar, the log of average ca and la, with s_RUSa as weights
* 

gen cbar_a = s_RUSa*ca
egen cbar = total(cbar_a)
di cbar
gen log_cbar = ln(cbar)

gen lbar_a = s_RUSa*la
...
gen logc_a = logca*s_RUSa
egen Elogc = total(logc_a)

gen logl_a = logla*s_RUSa
egen Elogl = total(logl_a)

**************************
gen epsl_a = eps_la*s_RUSa
...
scalar log_cbar_07=log_cbar
scalar eps_lbar_07=eps_lbar
scalar Elogc_07=Elogc
scalar Eeps_l_07=Eeps_l

scalar Cbar_RUS_07=exp(log_cbar_07) //This is 2007 micro data on consumption weighted by 2007 survival rates
scalar epsLbar_RUS_07=eps_lbar_07 //This is 2007 micro data on leisure weighted by 2007 survival rates
scalar Cineq_RUS_07=Elogc_07-log_cbar_07
scalar Lineq_RUS_07=-$theta*(Eeps_l_07-eps_lbar)
...
****** Lambda's are found in 2 steps because 1998 consumption and expenditure data are different ******
...
**open 1998 consumption data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\RUS\RUS_98.dta", clear
use "RUS_98.dta", clear
...
*Now that we have allocated expenditures to individuals, convert individual consumption from 
* 	1998 RUS to constant international prices, incorporate govt consumption
* 	using Penn World Tables 6.3, and scale to year $baseyear US $
...
**Compute stdev of ln(leisure) with RUS base period demographic weights
foreach var in lnchat {
qui {
su `var' [weight=s_RUSa*wbar]
return list
...
collapse (mean) logca=logca ca=ca s_RUSa=s_RUSa  delta_s_RUSa= delta_s_RUSa, by(country_code year age)
...
*generate log_cbar, the log of average ca, with s_RUSa as weights
...
scalar log_lambda_C_avg = log_cbar - ln(Cbar_RUS_07)


* Calculate consumption inequality term
scalar log_lambda_C_ineq = Elogc - log_cbar - Cineq_RUS_07
...
**open 1998 leisure data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\RUS\RUS_98.dta", clear
use "RUS_98.dta", clear
replace age=age+1
...
collapse (mean)logla=logla eps_la la=la s_RUSa=s_RUSa delta_s_RUSa=delta_s_RUSa, by(country_code year age)
...
scalar log_lambda_L_avg = -$theta*(eps_lbar - epsLbar_RUS_07)

* Calculate leisure inequality term
scalar log_lambda_L_ineq = -$theta*(Eeps_l-eps_lbar) - Lineq_RUS_07
...
* Lambda Decomposition for RUS 1998-2007
