...
*****************************************************************************
*		 FIND THE GROWTH LAMBDA IN INDONESIA BETWEEN 1993 - 2006				*
* This program computes the % reduction in consumption that individuals are *
* willing to undertake in 2006 and still be as well of as they were in 1993 *
*****************************************************************************

*** The program proceeds in 2 steps

*** STEP I: Calculate base (2006) values for CbarIDN, LbarIDN, CineqIDN, 
* LineqIDN etc using 2006 data and 2006 survival rates

*** STEP II, use 1993 data with 2006 survival rates and combine with Part I 
...
* 	2006 IDN to constant international prices, incorporate govt consumption
* 	using Penn World Tables 7.1, and scale to year $baseyear US $
...
if `bbbb'<1990 {
	replace year=1990
	merge m:1 country_code year age using "$survival_ind/survivalIDN`bb'.dta"
	keep if year==1990
	replace year=`bbbb'
}
else {
...
collapse (mean) logca=logca logla=logla eps_la=eps_la ca=ca la=la s_IDNa=s_IDNa  delta_s_IDNa= delta_s_IDNa, by(country_code year age)
...
*generate log_cbar and log_lbar, the log of average ca and la, with s_IDNa as weights
* 

gen cbar_a = s_IDNa*ca
egen cbar = total(cbar_a)
di cbar
gen log_cbar = ln(cbar)

gen lbar_a = s_IDNa*la
...
gen logc_a = logca*s_IDNa
egen Elogc = total(logc_a)

gen logl_a = logla*s_IDNa
egen Elogl = total(logl_a)

**************************
gen epsl_a = eps_la*s_IDNa
...
scalar Cbar_IDN_`bb'=exp(log_cbar_`bb') 
*This is 2006 micro data on consumption weighted by 2006 survival rates
scalar epsLbar_IDN_`bb'=eps_lbar_`bb' 
*This is 2006 micro data on leisure weighted by 2006 survival rates
scalar Cineq_IDN_`bb'=Elogc_`bb'-log_cbar_`bb'
scalar Lineq_IDN_`bb'=-$theta*(Eeps_l_`bb'-eps_lbar)
...
****** Lambda's are found in 2 steps because 1993 consumption and expenditure data are different ******
...
**open 1993 consumption data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\IDN\IDN_84.dta", clear
use "IDN_`aa'.dta", clear
format hhid %20.0f
egen weight_sum = sum(weight)
replace weight = weight/weight_sum
*ren hhexp exp
...
*Now that we have allocated expenditures to individuals, convert individual consumption from 
* 	1993 IDN to constant international prices, incorporate govt consumption
...
*sum weights within age groups, create wbar:
*(IDNividual sampling weight)/(sum of all IDNividual sampling weights in age group) 
sort age 
by age: egen total_age_weight=total(weight)
gen wbar=weight/total_age_weight 
drop weight total_age_weight
...
**Compute stdev of ln(leisure) with IDN base period demographic weights
foreach var in lnchat {
qui {
su `var' [weight=s_IDNa*wbar]
return list
...
collapse (mean) logca=logca ca=ca s_IDNa=s_IDNa  delta_s_IDNa= delta_s_IDNa, by(country_code year age)
...
*generate log_cbar, the log of average ca, with s_IDNa as weights
...
scalar log_lambda_C_avg = log_cbar - ln(Cbar_IDN_`bb')


* Calculate consumption inequality term
scalar log_lambda_C_ineq = Elogc - log_cbar - Cineq_IDN_`bb'
...
**open 1993 leisure data file. Discard age > 100 and renormalize weights
quietly {
*use "C:\Users\klenow\Documents\Rawls\IDN\IDN_84.dta", clear
use "IDN_`aa'.dta", clear
format hhid %20.0f
...
collapse (mean)logla=logla eps_la la=la s_IDNa=s_IDNa delta_s_IDNa=delta_s_IDNa, by(country_code year age)
...
scalar log_lambda_L_avg = -$theta*(eps_lbar - epsLbar_IDN_`bb')

* Calculate leisure inequality term
scalar log_lambda_L_ineq = -$theta*(Eeps_l-eps_lbar) - Lineq_IDN_`bb'
...
* Lambda Decomposition for IDN 1993-2006
