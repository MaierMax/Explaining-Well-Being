...
*global main_root C:\Users\Rui\Dropbox\Beyond_GDP
...
* need in 1993 to be as well of as they are in 2006 						*
*****************************************************************************

*** STEP I: Calculate base (1993) values for CbarIDN, LbarIDN, CineqIDN, 
* LineqIDN etc using 1993 data and 1990 survival rates. 
...
**********************************************************
*********				STEP I  			 *************
********* 1993 data with 1990 survival rates *************
**********************************************************
...
* 	`aaaa' INDONESIA rupiah to constant international prices, incorporate govt consumption
...
if `aaaa'<1990 {
	...
	*Merge with country-age level survival rates
	merge m:1 country_code year age using "$survival_ind/survivalIDN90.dta"
	*merge m:1 country_code year age using "C:\Documents and Settings\Pete Klenow\My Documents\My Documents\Rawls\Survival Rates\survivalIDN90.dta"
	keep if year==1990
	replace year=`aaaa'
}
else {
	merge m:1 country_code year age using "$survival_ind/survivalIDN`aa'.dta"
	*merge m:1 country_code year age using "C:\Documents and Settings\Pete Klenow\My Documents\My Documents\Rawls\Survival Rates\survivalIDN90.dta"
	keep if year==`aaaa'
}
keep if country_code=="IDN"
...
scalar log_cbar_`aa'=log_cbar
scalar Elogc_`aa'=Elogc

scalar Cbar_IDN_`aa'=exp(log_cbar_`aa')
scalar Cineq_IDN_`aa'=Elogc_`aa'-log_cbar_`aa'
...
*generate log_lbar, the log of average la, with s_IDNa as weights
* 
...
scalar eps_lbar_`aa'=eps_lbar
scalar Eeps_l_`aa'=Eeps_l

scalar epsLbar_IDN_`aa'=eps_lbar_`aa' 
scalar Lineq_IDN_`aa'=-$theta*(Eeps_l_`aa'-eps_lbar)
...
*Now that we have allocated expenditures to individuals, convert individual consumption from 
* 	2006 INDONESIA pesos to constant international prices, incorporate govt consumption
...
collapse (mean) logca=logca logla=logla eps_la la=la ca=ca s_IDNa=s_IDNa  delta_s_IDNa= delta_s_IDNa, by(country_code year age)
...
scalar log_lambda_C_avg = log_cbar - ln(Cbar_IDN_`aa')

* Calculate average leisure term
scalar log_lambda_L_avg = -$theta*(eps_lbar - epsLbar_IDN_`aa')

* Calculate consumption inequality term
scalar log_lambda_C_ineq = Elogc - log_cbar - Cineq_IDN_`aa'

* Calculate leisure inequality term
scalar log_lambda_L_ineq = -$theta*(Eeps_l-eps_lbar) - Lineq_IDN_`aa'
...
* Lambda Decomposition for IDN `aaaa'-`bbbb'
